...
| Policy-bypass attacker | Uses stale approval, wrong evidence, over-broad scope, or unbound parameters. | Execution when proof does not match exact action/audience/scope. | Business correctness of an action the issuer deliberately signed. | Binding and policy-refusal tests exist; docs require evidence binding. | Issuer/control-plane policy quality remains critical. | `tests/security/test_replay_escrow_attacks.py`, `tests/unit/test_preflight.py` |
